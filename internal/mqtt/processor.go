...
// Processor serializes MQTT commands and periodic refreshes through one worker
// so Bailup state changes are applied in order. Incoming work is bounded; when
// the queue is full, new MQTT commands are dropped instead of blocking callbacks.
